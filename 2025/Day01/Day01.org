...
#+author: Leo Laporte
#+date: Started: 30 Nov 2025 at 21:01, Finished: 1 Dec 2025 at 13:10
...
*Part One:* Given a list of safe dial turns (left and right), starting at position 50 on a dial numbered 0-99, count how many times the dial lands on position 0.

*Part Two:* Count how many times the dial passes through or lands on 0, including when turns go around multiple times.

* Approach

** Part 1
I'm counting the number of times the position = 0. I'll parse the input to replace R and L with + and - and just convert the instructions into positive and negative addends. Then use MOD to wrap around.

** Part 2
Now instead of counting the number of times the dial lands on zero I need to count the number of times it passes zero. Note that some of the turns go around multiple times. R1000 would pass zero as many as 10 times.

This took me way too long. And my brain still hurts.

* Setup
:PROPERTIES:
:header-args:lisp: :tangle Day01.lisp :comments link
:END:
...
** Parsing and Position Functions

I use these function type declarations from Serapeum to help keep me on the straight and narrow. (Plus they alert me to many dumb misteaks.)
...
* Part 2: Count Times Dial Passes Through Zero
