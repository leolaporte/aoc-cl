#+title: AOC 2025 Day 2: Invalid IDs
#+author: Leo Laporte
#+date: Started: 01 Dec 2025 at 21:00, Finished: 01 Dec 2025 at 22:40
#+property: header-args:lisp :session *sly-session* :results output :exports both :eval yes
#+auto_tangle: t

* Problem Description

Day 2 of Advent of Code 2025: Invalid IDs

*Part One:* Given a string with a list of number ranges, add up the invalid IDs in those ranges. An ID is invalid if it is "made only of some sequence of digits repeated twice."

*Part Two:* Find IDs that consist of a repeating digit sequence (repeated one or more times, not just twice).

* Approach

** Part 1
We're given a string with a list of number ranges. We're to add up the invalid IDs in those ranges. An ID is invalid if it is "made only of some sequence of digits repeated twice." Sounds like a job for regular expressions.

Regex for a doubled sequence would look like this: =^(\\d+)\\1$=

=(\\d+)= is a group that contains one or more digits, and the =\\1= means repeat that group, i.e. a doubled sequence. The =^= and =$= are anchors for the beginning and end of the string, thus we get a doubled sequence and ONLY a doubled sequence, no extraneous characters. Sorry about the =\\= - Common Lisp requires the =\= to be escaped... with =\=. In Perl or Python it would just be =^(\d+)\1$=

Ah interesting refinement. Paul Holder asked about using lazy evaluation in the regex instead. =\d+?= instead of =\d+= - and indeed it sped the search up in part 2 by 28 milliseconds. I don't think that's always going to be the case but because I am performing the SCAN millions of times in both parts it's a nice little improvement. Thanks, Paul!

** Part 2
This one's easy. Just change the regex to =^(\\d+)\\1+$= (adding the plus after the 1 to mean the group can be repeated one or more times) - I'll refactor part one to allow plug-in regexes and we're done. (cl-ppcre's CREATE-SCANNER compiles the regex string into a function which makes it somewhat faster).

* Setup
:PROPERTIES:
:header-args:lisp: :tangle Day02.lisp :comments link
:END:
...
** Example Data and Regex Patterns
...
* Part 1: Sum Invalid IDs (Twice-Repeated Sequences)
...
* Part 2: Sum Invalid IDs (Multiple-Repeated Sequences)
...
I can reuse part 1 entirely with the new regex!
