...
instructions"
  (labels ((contains-move? (s) (equal (first (uiop:split-string s)) "move"))) ; kinda brute force - look for "move"
...
(defun build-stacks (stack-strings)
  ...
  (do* ((dim (parse-integer
    ...
    (dotimes (i dim)  ; for each stack in stack-array
      (progn
	(setf letter (subseq (first ss) (1+ (* i 4)) (+ (* i 4) 2))) ; ugly, skip [] and spaces, jump by 4
...
(defun execute-instructions (stacks insts)
  ...
  (dolist (i insts)                                  ; do this for each instruction in the list
	...
	(push (pop (aref stacks (- source 1))) (aref stacks (- dest 1))))))) ; the actual move (a pop and a push)
...
this time in chunks instead of one at a time"
  (dolist (i insts)                                                         ; do the following for each move
	...
	(*inst-string* i)
      ...
      (do ((chunk (reverse (subseq (aref stacks (1- source)) 0 how-many))   ; get the whole chunk
		  (rest chunk)))                                            ; as a reversed list
	  ((null chunk))                                                    ; return when we've pushed the whole chunk
	(progn
