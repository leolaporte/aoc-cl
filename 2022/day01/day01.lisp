...
(defparameter *data-file* "~/cl/AOC/2022/day01/input.txt")  ; AoC input
...
NOTES: As is often the case the only challenge here is how to parse the
data. FILE-READ-LINES reads the file into a list of integer strings separated by
an empty string (that's how it interprets the blank line). The obvious solution
is nested for loops - one to total the groups, one to stuff the totals into a
list. DO loop to the rescue.

This is a uniquely lispy idiom I use all the time. The DO has three clauses, the
first sets the local variables (and advances them each time through the loop),
the second is the termination case with a return value, the third is the body of
the loop.  |#
...
return the list of total calories carried by each elf sorted from greatest to lowest"
  (do ((loc list-of-cals (rest loc))      ; the input data, a list of calorie strings
	...
	;; else
	(incf cals-per-elf (parse-integer (first loc)))))) ; add to elf's total
...
(defun day01-1 (f)
  "given a data file containing a list of integer string groups separated by empty
strings return the highest group total"
  (first (calorie-totals (uiop:read-file-lines f))))
...
(defun day01-2 (f)
  ...
  (reduce  #'+ (calorie-totals (uiop:read-file-lines f)) :end 3)) ; add top three

;; Now solve the problem!

(time (format t "The answer to AOC 2022 Day 01 Part 1 is ~a"
	      (day01-1 *data-file*)))

(time (format t "The answer to AOC 2022 Day 01 Part 2 is ~a"
	      (day01-2 *data-file*)))
