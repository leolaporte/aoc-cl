...
(defparameter *data-file* "~/cl/AOC/2024/day15/input.txt"
...
the edges of the map.)nnnnnnnnnnnnnnn
...
(5a:test next-positions-test
  (multiple-value-bind (map width pos move-instructions)
    ...
    ;; the no move possible bunch...
    (5a:is (equal '(12) (next-positions map *up* 12 width)))
    (5a:is (equal '(17) (next-positions map *right* 17 width)))
    (5a:is (equal '(65) (next-positions map *down* 65 width)))
    (5a:is (equal '(62) (next-positions map *left* 62 width)))))
...
Hoo boy. I can easily modify the map using MAKE-FAT-MAP.

So horizontal moves don't change. it's the up and down moves that need
some pondering. The robot can push either edge of a box to move it,
and presumably a rock can block either edge, too. So the way has to be
clear for both edges.

So the difference here is that a rock can be represented as [ or ] and
if I move one part both have to move, and be able to move. So look
ahead has to include two open points - if the robot is adjacent to [
then robot col and col + 1 have to be open above. and ] robot col and
robot col - 1 must be open. But wait what about when the left edge of
a box catches the right edge of the box above it? Now robot col -1 and
-2 must be open, and so forth. This is getting kind of complicated.

So here is an example of the kind of calculation SLIDE-2 will have to
address:
...
In this case @ can ^ giving the resulting map:
...
But in this case no move is possible
...
So for every BOX in the slide there has to be either a free space or
another box in the spaces above it, and the final box must have two
free spaces above it. In other words all the boxes in the chain have
to have somewhere to go. I will have to check BOTH next positions. For
the robot that's (next-position ROBOT *up*) and (next-position (1+
ROBOT) *up*). And then for boxes I'll have to find what's above #\[
and #\] (that might be 1+ or 1-).

Also the GPS rules are modified, so I'll need a new (more complicated)
CALCULATE-FAT-GPS routine.

---------------------------------------------------------------------------- |#

(defun make-fat-map (map)
  "turn old style map into new, fatter, map - the map vector is just a
string so this is simple."
  (setf map (str:replace-all "#" "##" map))
  (setf map (str:replace-all "O" "[]" map))
  (setf map (str:replace-all "." ".." map))
  map)

(5a:test make-fat-map-test
  (5a:is (string= (make-fat-map "#.O.#") "##..[]..##"))
  (5a:is (string= (make-fat-map (parse-warehouse *big-example*))
                  (remove #\Newline (remove #\Space
                                            "####################
                                             ##....[]....[]..[]##
                                             ##............[]..##
                                             ##..[][]....[]..[]##
...
(defun day15-2 (los)
  "given a new, fatter, warehouse map and a list of moves return the GPS
positions of all the *BOX*es after executing all the moves"
  (multiple-value-bind (map width pos moves)
      (parse-warehouse los)

    (setf map (make-fat-map map pos))
